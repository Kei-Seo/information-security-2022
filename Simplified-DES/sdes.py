...
def sdes(text: bitarray, key: bitarray, mode) -> bitarray:
    result = bitarray()
    roundKey_list = schedule_keys(key)
    s_text = text
    if mode == 1:
        IP_Text = bitarray()
        for i in IP:
        ...
        # 좌 우를 4bit 로 나눈다
        L_IP_Text = IP_Text[0:4]
        R_IP_Text = IP_Text[4:8]

        round0 = round(R_IP_Text, roundKey_list[0])

        temp = R_IP_Text
        R_IP_Text = L_IP_Text ^ round0
        L_IP_Text = temp

        round1 = round(R_IP_Text, roundKey_list[1])
        L_IP_Text = round1 ^ L_IP_Text
        output = L_IP_Text + R_IP_Text
        ...
        round0 = round(R_IP_Text, roundKey_list[1])

        temp = R_IP_Text
        R_IP_Text = L_IP_Text ^ round0
        L_IP_Text = temp

        round1 = round(R_IP_Text, roundKey_list[0])
        L_IP_Text = round1 ^ L_IP_Text
        output = L_IP_Text + R_IP_Text
